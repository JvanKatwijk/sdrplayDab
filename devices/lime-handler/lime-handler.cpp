...
//	default frequency
	...
	connect (this, SIGNAL (showOffset (int)),
	         this, SLOT (displayOffset (int)));
	connect (this, SIGNAL (showGains (float, float)),
	         this, SLOT (displayGains (float, float)));
	running. store (false);
}
...
void	limeHandler::displayOffset	(int offset) {
	offsetDisplay	-> display (offset);
}

void	limeHandler::displayGains	(float nullValue, float avgValue) {
	float v1 = 20 * log10 ((nullValue + 0.005) / 2048.0);
	float v2 = 20 * log10 ((avgValue + 0.005) / 2048.0);
	nullValueDisplay	-> display (v1);
	avgValueDisplay		-> display (v2);
}
	   ...
	   if (res > 0) {
	      ...
	      for (int i = 0; i < res; i ++) {
	         std::complex<float> symb =
	                   std::complex<float> (localBuffer [2 * i],
	                                        localBuffer [2 * i + 1]);
	         int res	= base -> addSymbol (symb);
	         if (res == GO_ON)
                    continue;
	            ...
	            base -> update_data (&offset, &lowVal, &highVal);
                    if (++ teller > 20) {
                       emit showOffset (offset);
	               emit showGains (lowVal, highVal);
	               if (offset != 0) {
	                  float_type freq;
	                  int res = LMS_GetLOFrequency
	                          (theDevice, LMS_CH_RX, 0, &freq);
	                  LMS_SetLOFrequency
	                          (theDevice, LMS_CH_RX, 0, (float_type)(freq + offset));
	               }
	            }
	            else
...
void    limeHandler::setEnv  (dabProcessor *p) {
	fprintf (stderr, "env is set\n");
        base    = p;
}

void    limeHandler::show    (void) {
        myFrame         -> show ();
}

void    limeHandler::hide    (void) {
        myFrame         -> hide ();
}

bool    limeHandler::isVisible       (void) {
